...
function createAdvantagesGallery(elementId, jsonData) {
  var galleryContainer = $("#" + elementId);
  jsonData.forEach(function (item) {
    var linkElement = $("<a href=\"".concat(item.image, "\" title=\"").concat(item.text, "\" data-gallery><img src=\"").concat(item.image, "\" alt=\"").concat(item.text, "\"></a>"));
    galleryContainer.append(linkElement);
  }); // Инициализация Blueimp Gallery

  $("#links").on("click", "a", function (event) {
    event.preventDefault();
    blueimp.Gallery($("#links a").toArray(), {
...
document.getElementById('search-form').addEventListener('submit', function (event) {
  ...
  if (query) {
    var filteredProducts = products.filter(function (product) {
      return typeof product.ID_EXT === 'string' && product.ID_EXT.toLowerCase().includes(query) || typeof product.zapchast === 'string' && product.zapchast.toLowerCase().includes(query) || typeof product.markaavto === 'string' && product.markaavto.toLowerCase().includes(query) || typeof product.model === 'string' && product.model.toLowerCase().includes(query) || typeof product.category === 'string' && product.category.toLowerCase().includes(query) || typeof product.dop_category === 'string' && product.dop_category.toLowerCase().includes(query) || typeof product.originalnumber === 'string' && product.originalnumber.toLowerCase().includes(query);
    });
